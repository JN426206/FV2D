...
class HomographyEstimator:
    ...
    def __init__(
        ...
        self.shape_in_x = shape_in_x
        self.shape_in_y = shape_in_y
        self.shape_out = shape_out
        self.ratio_x = shape_out / shape_in_x
        self.ratio_y = shape_out / shape_in_y

    def __call__(self, input_img, treshold = 0.9):
        pr_mask = self.keypoints_model(input_img)
        src, dst = _points_from_mask(pr_mask[0], treshold)
        if len(src) > 3:
    ...
    def get_field_coordinates(self, bbox, pred_homo, method):
        ...
        self.ratio_x = self.shape_out / self.shape_in_x
        self.ratio_y = self.shape_out / self.shape_in_y
        x_1 = int(bbox[0])
        y_1 = int(bbox[1])
        x_2 = int(bbox[2])
        y_2 = int(bbox[3])
        x = (x_1 + x_2) / 2.0 * self.ratio_x
        y = max(y_1, y_2) * self.ratio_y
        if method == "cv":
